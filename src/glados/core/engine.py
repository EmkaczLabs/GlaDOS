...
class Glados:
    ...
    def submit_text(self, text: str) -> None:
        """
        Submit typed text to the LLM queue.

        This method behaves similarly to processing transcribed ASR text: it
        respects the assistant's interruptibility settings, attempts to stop
        any ongoing speech playback (if interruptible), and then places the
        provided text onto the LLM input queue for the language model to
        process.

        Args:
            text: The user-provided input text to be handled by the assistant.
        """
        text = text.strip()
        if not text:
            logger.debug("submit_text called with empty text; ignoring.")
            return

        # If the assistant is not interruptible and is currently speaking, ignore input
        if not self.interruptible and self.currently_speaking_event.is_set():
            logger.info("submit_text ignored: assistant is currently speaking and not interruptible.")
            return

        # Stop any current speech playback and signal we will (re)start processing
        try:
            self.audio_io.stop_speaking()
        except Exception:
            logger.debug("submit_text: audio_io.stop_speaking() raised an exception; continuing.")

        # Clear any previous processing flag then submit the new request
        self.processing_active_event.clear()
        logger.success(f"Typed input: '{text}'")
