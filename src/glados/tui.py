...
class TypeInputScreen(ModalScreen[None]):
    """A modal screen that allows the user to type a message for the assistant.

    The submitted text is routed to the running Glados engine (if available)
    via its new `submit_text` method so that typed input behaves like ASR
    input (i.e., it will be spoken and added to the conversation history).
    """
    ...
    def compose(self) -> ComposeResult:
        yield Container(Label("Type message and press Enter", id="type_label"), Input(placeholder="Type message...", id="type_input"), id="type_container")
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if not text:
            app.notify("Empty input ignored", title="GLaDOS", severity="warning")
            return

        if app.glados_engine_instance is None:
            app.notify("GLaDOS engine not ready yet", title="GLaDOS", severity="warning")
            return

        try:
            app.glados_engine_instance.submit_text(text)
            app.notify("Input sent", title="GLaDOS")
        except Exception as e:
            logger.opt(exception=True).error(f"Failed to send typed input: {e}")
            app.notify("Failed to send input", title="GLaDOS", severity="error")
        finally:
            self.dismiss()
...
class GladosUI(App[None]):
    ...
    BINDINGS: ClassVar[list[Binding | tuple[str, str] | tuple[str, str, str]]] = [
        ...
        Binding(key="t", action="type", description="Type message"),
    ]
    ...
    def action_type(self) -> None:
        """Open the typing modal screen."""
        self.push_screen(TypeInputScreen(id="type_screen"))
